...
matplotlib.rcParams['figure.figsize'] = [183./25.4,11]
...
matcolordict = {'GaAs' : 0, 'AlGaAs': 1, 'GaInP': 2, 'InGaAsN':3, 'Germanium':4, 'Gold': 5, 'TiOx':6, 'MgF':7, 'InGaAsN': 8, 'AlInP':9, 'AlOx':6, 'AlGaInP':9}
...
class Plots:
    def __init__(self):
        # Figure and Axes
        self.p1 = plt.figure(figsize=(7,4))
        gs = matplotlib.gridspec.GridSpec(3,1,height_ratios=[0.5,6,6])
...
#        self.ax2.clear()
        for i in epi.layers:
            ...
            pat2 = matplotlib.patches.Rectangle((i.ytop,ylim2[0]),i.ybot-i.ytop,ylim2[1]-ylim2[0], color=colours[matcolordict[i.material]], alpha=0.1)
            
            self.ax0.add_patch(pat0)
            self.ax1.add_patch(pat1)
            self.ax2.add_patch(pat2)
...
#        plots.ax1.set_ylim(-7,1)
	plots.ax2.set_ylim(1e19,1e25)
	plots.ax2.set_yscale('log')
	plots.ax2.set_xlabel('position Z ($\mu$m)')
	...
	plots.ax2.set_ylabel(r'\centering Recombination Rates\\($\mathsf{cm^{-3}s^{-1}}$)\\')

	ticks = plots.ax1.yaxis.get_ticklines()
        ticks[0].set_visible(False)
        ticks[-1].set_visible(False)
        ticks = plots.ax2.yaxis.get_ticklines()
        ticks[0].set_visible(False)
        ticks[-1].set_visible(False)
	...
	ticks = plots.ax1.xaxis.get_ticklines()
        ticks[0].set_visible(False)
        ticks[-1].set_visible(False)
	ticks = plots.ax2.xaxis.get_ticklines()
        ticks[0].set_visible(False)
        ticks[-1].set_visible(False)
