...
colours = matplotlib.rcParams['axes.color_cycle']*2
...
matplotlib.rcParams['figure.figsize'] = [183./25.4,7]
...
matcolordict = {'GaAs' : 0, 'AlGaAs': 1, 'GaInP': 2, 'InGaAsN':3, 'Germanium':4, 'Gold': 5, 'TiOx':6, 'MgF':7, 'InGaAsN': 8, 'AlInP':9, 'AlOx':6, 'AlGaInP':9, 'MQW_eff': 4}
...
class Plots:
    def __init__(self):
        ...
        gs = matplotlib.gridspec.GridSpec(2,1,height_ratios=[0.25,5])
...
#	plot_from_csv(plots.ax2, svis, 'AugerRecombination',  color=rec_color, dashes=(1,1), label='Auger Rec.')
	plot_from_csv(plots.ax1, svis, 'OpticalGeneration',  color='k', label='External Gen.')
...
#        plots.ax1.set_yscale('log')
        
	plots.set_hscale()

	plots.ax1.set_ylabel(r'\centering Geenration Rate ($\mathrm{cm^{-3}s^{-1}}$)\\')
	...
	ticks = plots.ax1.xaxis.get_ticklines()
        ...
        of = plots.ax1.get_yaxis().get_offset_text()
        print of
        of.set_position((-0.08,1.05))
