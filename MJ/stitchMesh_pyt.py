...
datFile = '../@node|tdx@/dfise_file.dat'
grdFile = '../@node|tdx@/dfise_file.grd'
...
outputFile = 'optgen.dat'
outputGrid = 'optgen.grd'
...
with open(outputFile,"w") as fid:
	...
	for regionName in regionCell.keys():
		...
		for nOG in range(len(regionCell[regionName].x)):
			fid.write(' ' +str(optGen(regionCell[regionName].x,regionCell[regionName].y,regionCell[regionName].z)))
...
print('\n\nSimulation time: ' + "{:.4}".format(time.time() - t0) + ' s')
