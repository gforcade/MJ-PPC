...
norm = matplotlib.colors.LogNorm(vmin=1e-6, vmax=1, clip=False)
...
with open('n' + str(LC_node) + '_LC.csv') as infile:
...
cbar = p1.colorbar(cs, cax=ax3, orientation='horizontal', ticks=[1e-4, 1e-3, 1e-2, 1e-1, 1], format=matplotlib.ticker.LogFormatterMathtext(10, labelOnlyBase=True))
