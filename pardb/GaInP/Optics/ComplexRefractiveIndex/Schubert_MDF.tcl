...
#! NOTE: The calculation gives a k value too large for energies less than band gap.  We force it to zero.
#! Also, we use the parameter Eg_GaInP to shift the results over a small range.
...
namespace eval GaInP::ComplexRefractiveIndex {
	...
	# For GaInP we use (Al) mole fraction=0
	set xMole 0
	...
	variable Formula 1
       ...
       # The reference provides data for disordered GaInP with band gap = 1.899 eV.  we'll apply a shift
       # in energy to allow for other band gaps due to ordering. Granted, this is not very 'physical', but
       # at least it puts the band edge where we want it.

       set E_shift [expr $::gparam(Eg_GaInP) - $E0]
	...
	for { set l 0.2 } { $l < 2.0 } { set l [expr $l + 0.005] } {
		...
		# force k to zero for energy less than band gap
		if { $E < $E0 } {
			set k 0.0
		}
