...
(Needed for running the pyPrepper in the gsub prologue: Swb automatically creates the _pyt.py file at the start of every run of gsub.)

Operation;
Add
set WB_tool(gsub,prologue) { exec  python $wdir/path/to/py2prepper.py $wdir }
to the project tooldb
...
config.py
(Config can be recoupled if it is made py2-friendly.)
