...
def grabIVParams(fileName):
	...
	keys = data.keys()
	Jsc_func = interpolate.interp1d(data[keys[0]],data[keys[1]],fill_value="extrapolate")
	Jsc = Jsc_func(0.0)
	Voc_func = interpolate.interp1d(data[keys[1]],data[keys[0]],kind='cubic',fill_value="extrapolate")
	Voc = Voc_func(0.0)
	P_func = interpolate.interp1d(data[keys[0]],-(data[keys[1]]*data[keys[0]]),kind='cubic')
	res = optimize.minimize_scalar(P_func,bounds=[0.0,data[keys[0]].max()],method='bounded')
	Pmax = -res.fun
	FF = Pmax * 100.0 / (Voc * Jsc) 
