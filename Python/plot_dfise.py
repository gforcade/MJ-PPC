...
class ControlPanel(QtGui.QWidget):
    ...
    def set_hscale(self):
        ...
        for i in epi.layers:
            if i.xmole > 0.0:
                label = i.material + ' $x=' + str(i.xmole) + '$'
            else:
                label = i.material
            ...
            t = matplotlib.textpath.TextPath((0,0),label,rotation='vertical', va='bottom', ha='center',size=10)
...
#            print bb1.width, bb2.height
            if bb2.height*6.0 < bb1.width-2:
                ...
                self.ax0.text((i.ytop+i.ybot)/2,0.05,label,rotation='vertical', va='bottom', ha='center',multialignment='left',size=10, clip_on=True)
        self.canvas.draw()
...
#        gs = matplotlib.gridspec.GridSpec(4,1,height_ratios=[1.75,4,4,4], hspace=0.1)
        gs = matplotlib.gridspec.GridSpec(4,1,height_ratios=[1,1,2,6], hspace=0.2, left=0.1, right=0.95, top=0.95, bottom=0.1)
        self.ax3 = self.p1.add_subplot(gs[0,0])

        self.ax2 = self.p1.add_subplot(gs[1,0])
        self.ax0 = self.p1.add_subplot(gs[2,0])
        self.ax1 = self.p1.add_subplot(gs[3,0])
